...
class Node:
    def __init__(self, value, data, color, parent, left=None, right=None):
        self.value = value
        self.data = data
        self.color = color
        self.parent = parent
    ...
    def __repr__(self):
        return 'Node: {color} {val} {data}'.format(color=self.color, val=self.value, data=self.data)
...
class RedBlackTree:
    ...
    NIL_LEAF = Node(value=None, data=None, color=NIL, parent=None)
    ...
    def add(self, value, data=None):
        if not self.root:
            self.root = Node(value=value, data=data, color=BLACK, parent=None, left=self.NIL_LEAF, right=self.NIL_LEAF)
            self.count += 1
            return
        parent, node_dir = self._find_parent(value)
        if node_dir is None:
            return  # value is in the tree
        new_node = Node(value=value, data=data, color=RED, parent=parent, left=self.NIL_LEAF, right=self.NIL_LEAF)
        if node_dir == 'L':
...
## cnt = 0
while True:
    ## print('You were in loop for {cnt} seconds!'.format(cnt=cnt))
    ## time.sleep(1)
    ## cnt += 1
    pass
